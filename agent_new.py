...
class Agent():
    ...
    def find_diff(lists):
        ...
        for i in range(len(lists) - 1):
            x_diff, y_diff = lists[i+1][0] - lists[i][0], lists[i+1][1] - lists[i][1]
            temp.append(np.sqrt(x_diff**2 + y_diff**2))
        return temp
    ...
    def run_step(self, filtered_obstacles, waypoints, vel, transform, boundary):
            ...
            # if filtered_obstacles != []:
                ...
                if obs_distance <= 15: # time to do avoiding ///15: the distance from the closest obstacle to start avoiding program
                    ...
                    if len(self.paths) != 0:
                        ...
                        self.new_waypoints = self.paths.reverse()
                    self.mode = 0
                else:
        ...
        if self.mode == 0:
            ...
            #self.new_waypoints = self.paths.reverse()
            print('diff in inserted waypoints is, ', self.find_diff(self.new_waypoints))
            ...
            if self.dist < 5: #might have to tune
                self.counter += 1
                self.new_waypoints.pop(self.new_waypoints[0])
